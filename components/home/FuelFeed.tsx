...
export default function FuelFeed() {
  ...
  return (
    <div className="card p-4">
      <p className="text-text-muted text-[10px] uppercase tracking-widest font-bold mb-3">Activity</p>
        ...
        {feedItems.slice(0, 6).map(item => (
          <div key={item.id} className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-text-muted mt-0.5">{timeAgo(item.timestamp)}</p>
            </div>
          </div>
        ))}
      </div>
