  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="card p-4">
        <h3 className="font-headline font-bold text-white mb-1">What if you drove something different?</h3>
        <p className="text-text-secondary text-sm">
          Currently spending <span className="text-white font-headline font-bold">${currentWeekly.toFixed(0)}/week</span> on your {car?.make} {car?.model}
        </p>
      </div>
      ...
      {alternatives.map((alt) => {
        ...
        return (
          <div key={alt.model} className="card p-4">
            <div className="flex items-center justify-between mb-2">
              <div>
                <p className="font-headline font-bold text-sm text-white">{alt.make} {alt.model}</p>
                <span className="text-xs px-2 py-0.5 rounded-full bg-surface-high text-text-muted font-headline uppercase tracking-widest">
                  {alt.label}
                </span>
              </div>
              <div className="text-right">
                <p className="text-text-muted text-xs">{alt.type === 'ev' ? 'Electric' : `${alt.economyL100km}L/100km`}</p>
                <p className="font-headline font-bold text-sm text-white">${altWeekly.toFixed(0)}/week</p>
              </div>
            </div>
            <div className="bg-surface-high rounded-xl p-3">
              <p className="text-success font-headline font-bold text-sm">
